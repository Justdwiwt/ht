...
(function () {
    ...
    function Z(_, $) {
        ...
        F();
        if ((/WebKit|KHTML|MSIE/i).test(navigator.userAgent)) C();
        ...
        function C() {
            if (B.body !== null && B.getElementsByTagName) {
                ...
                if (G("readyState") && (/loaded|complete/).test(B.readyState)) E("readyState")
            }
            if (!D) setTimeout(C, 10)
        }

        function A() {
            if (typeof B.removeEventListener == "function") B.removeEventListener("DOMContentLoaded", E, false)
        }

        function F() {
            if (typeof B.addEventListener == "function") B.addEventListener("DOMContentLoaded", E, false);
            var $ = _.onload;
            _.onload = function (_) {
                if (typeof $ == "function") $();
                E(_ || this.event)
    ...
    function U(E) {
        if (R) return E.getBoundingClientRect(); else {
        ...
        if ($dp && $dp.dd && $dp.dd.style.display == "block" && src != $dp.el) {
            ...
            if ($ != null) {
                ...
                if ($ == "" || (B.isDate(B.date) && B.isTime(B.date) && B.checkValid(B.date))) {
                    B.mark(true);
                    if ($ != "") B.update(); else B.setRealValue("");
                    $dp.hide()
                } else B.mark(false)
            } else $dp.hide()
        }
    ...
    function S(C, $) {
        ...
        if ($) {
            ...
            if ($dp.status == 0) $dp.status = 1; else if ($dp.status != 2) return;
            var B, A = _();
            if (A) {
    ...
    function I(G, A) {
        ...
        for (F in G) if ($dp[F] === undefined) $dp.errMsg = "invalid property:" + F; else $dp[F] = G[F];
        ...
        if (!$dp.dd || $dp.eCont || ($dp.lang && $dp.realLang && $dp.realLang.name != $dp.lang)) {
            ...
            if ($dp.eCont) {
                ...
                if (A) $dp.dd.style.left = $dp.dd.style.top = "-1970px"; else {
                    $dp.show();
                    C()
                }
            }
        ...
        function C() {
            var F = $dp.position.left, B = $dp.position.top, G = U($dp.el), $ = E(V), C = M(T), A = Y(T),
                D = $dp.dd.offsetHeight, _ = $dp.dd.offsetWidth;
            if (isNaN(B)) {
                if (B == "above" || (B != "under" && (($.topM + G.bottom + D > C.height) && ($.topM + G.top - D > -5)))) B = A.top + $.topM + G.top - D - 3; else B = A.top + $.topM + G.bottom;
                B += R ? -1 : 1
            } else B += A.top + $.topM;
            if (isNaN(F)) F = A.left + Math.min($.leftM + G.left, C.width - _ - 5) - (R ? 2 : 0); else F += A.left + $.leftM;
