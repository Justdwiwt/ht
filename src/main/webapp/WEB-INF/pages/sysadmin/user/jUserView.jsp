        ...
        <div class="eXtremeTable">
            <table id="ec_table" class="tableRegion" width="98%">
                        ...
                        <select name="userInfo.manager.userInfoId" style="width:120px" disabled="disabled">
                            ...
                            <c:forEach items="${managerList}" var="p">
                                ...
                                <option value="${p.userInfoId}"
                                        <c:if test="${p.userInfoId == user.userInfo.manager.userInfoId}">selected="selected"</c:if>>${p.name}</option>
                            </c:forEach>
                        </select>
                    </td>
                </tr>
                ...
                <tr class="odd">
                    ...
                    <td>
                        <select name="userInfo.userLevel" style="width:120px" disabled="disabled">
                            ...
                            <option value="1" <c:if test="${user.userInfo.userLevel==1}">selected="selected"</c:if>>
                                总经理
                            </option>
                            <option value="2" <c:if test="${user.userInfo.userLevel==2}">selected="selected"</c:if>>副总
                            </option>
                            <option value="3" <c:if test="${user.userInfo.userLevel==3}">selected="selected"</c:if>>
                                部门经理
                            </option>
                        </select>
                    </td>
                ...
                <tr class="odd">
                    <td>备注信息:</td>
                    <td colspan="3">
                        <textarea name="userInfo.remark"
                                  style="height:80px;width:100%">${user.userInfo.remark}</textarea>
                    </td>
                </tr>
            </table>
        </div>
